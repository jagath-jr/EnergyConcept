(function () {
  ...
  function applyContact(data) {
    ...
    setText('.cp-card a[href^="tel:"]', data.contact.phone || data.global.phone);
    setAttr('.cp-card a[href^="tel:"]', 'href', `tel:${(data.contact.phone || data.global.phone).replace(/\s+/g, '')}`);
    setText('.cp-card a[href^="mailto:"]', data.contact.email || data.global.email);
    setAttr('.cp-card a[href^="mailto:"]', 'href', `mailto:${data.contact.email || data.global.email}`);
    const addr = document.querySelector('.cp-card__text');
    if (addr) addr.innerHTML = (data.contact.address || data.global.address).replace(',', ',<br>');
